...
│   │   ├── api/                     #   🌐 API 路由层
│   │   │   ├── datasets.py           #     内置数据集扫描（含嵌套）
│   │   │   ├── jobs.py               #     发起/查询/SSE 进度/取消
│   │   │   ├── models.py             #     模型库 CRUD + 文件流服务
│   │   │   ├── uploads.py            #     图片/ZIP 上传解压
│   │   │   ├── import_model.py       #     导入已有 ply/splat/ksplat
│   │   │   └── fs.py                 #     路径建议/可写校验
│   │   ├── core/                    #   ⚙️ 核心层
│   │   │   ├── scheduler.py          #     单队列串行任务调度
│   │   │   ├── events.py             #     SSE 事件总线（发布/订阅）
│   │   │   └── registry.py           #     models.json 注册表
│   │   ├── services/                #   🔧 服务层
│   │   │   ├── sfm_service.py        #     封装 COLMAP SfM
│   │   │   ├── trainer_service.py    #     调用训练脚本 + 进度解析
│   │   │   └── compressor.py         #     产物整理
│   │   └── utils/                   #   🛠️ 工具层
│   │       ├── proc.py               #     异步子进程 + 行级进度回调
│   │       └── file_handler.py       #     上传保存/ZIP 解压
│   └── train/                       #   🧠 自研训练器
│       ├── train_gsplat.py           #     精简 3DGS 训练脚本
│       └── colmap_io.py              #     自包含 COLMAP 二进制读取器
├── 🎨 frontend/                     # 前端可视化
...
│       ├── pages/                    #   📄 页面
│       │   ├── GalleryPage.tsx        #     模型库
│       │   ├── ReconstructPage.tsx    #     上传/重建/进度
│       │   ├── ViewerPage.tsx         #     全屏查看器
│       │   └── ComparePage.tsx        #     并排对比 + 消融表
│       ├── components/               #   🧩 组件
│       │   ├── SplatViewer.tsx        #     ★ 三模式高斯查看器核心
│       │   ├── UploadZone.tsx         #     拖拽上传
│       │   ├── JobProgress.tsx        #     SSE 进度条
│       │   └── PathPicker.tsx         #     保存路径选择
│       ├── api/                      #   🌐 client + sse 封装
│       └── store/                    #   zustand 全局状态
...
<div align="center">图 6-1　模型库主界面（建议截图 http://localhost:5173）</div>

> 展示模型卡片网格（内置/生成/导入三类），含格式、来源、PSNR 标注，右上角"导入本地文件"与"新建重建"入口。

<div align="center">图 6-2　新建重建页与 SSE 进度条（建议截图 /reconstruct 发起任务后）</div>

> 展示数据集选择/拖拽上传区、步数预设（3k/7k/30k）、保存路径选择，以及 SfM→训练→导出 三阶段实时进度条。

<div align="center">图 6-3　三维查看器环视模式（建议截图机车模型）</div>

> 展示 TandT 机车 30k 正则版模型，左键旋转/右键平移/滚轮缩放，右上角显示 FPS 与高斯数，底部三模式切换与剔除杂点滑块。

<div align="center">图 6-4　第一人称漫游模式（建议截图 playroom 室内场景）</div>

> 展示室内房间场景切换至"漫游"模式，从房间内部起步，WASD 移动 + 拖拽转视角。

<div align="center">图 6-5　引擎参数对比页（建议截图 /compare）</div>
...
实验表明，对此类无边界室外复杂场景，30,000 步是达到"清晰可读"质量的必要训练量。三种步数的重建效果对比建议截图，如图 6-6 所示。

<div align="center">图 6-6　训练步数消融对比图（建议并排截图 3k/7k/30k 三模型同视角）</div>

> 由模糊到清晰的渐进对比，直观体现训练步数对重建质量的影响。

PSNR 随训练步数的变化曲线如图 6-7 所示。

<div align="center">图 6-7　PSNR 随训练步数变化曲线（建议由训练日志绘制）</div>
...
引入正则项后，在 PSNR 不降反升（25.5→26.1）的同时，剔除约 20% 冗余漂浮高斯（281,406→226,174），模型体积下降约 21%（63→50 MB），网络传输与前端渲染负担同步降低。这表明正则项抑制的确为无效漂浮尖刺，而非有效的重建细节。有无正则项的视觉对比建议截图，如图 6-8 所示。

<div align="center">图 6-8　MCMC 正则项消融对比图（建议并排截图有/无正则 30k 模型同视角）</div>

> 左图（无正则）满天飞散尖刺，右图（有正则）残片显著减少，直观验证正则项的治本作用。

## 6.5 端到端重建成果

系统成功打通 `图像 → COLMAP 稀疏重建 → 自研 gsplat 训练 → PLY 导出 → 浏览器 WebGL 实时渲染` 完整闭环。以 tandt/train 机车场景为例，30k 正则版重建出清晰可辨的 Western Pacific 机车（车身文字、铁轨、道砟均清晰重建），渲染帧率稳定 40 FPS 以上，验证了系统的实用性与实时性。端到端重建成果建议截图，如图 6-9 所示。

<div align="center">图 6-9　端到端重建成果图（建议截图机车模型多角度）</div>
...
> **附：图表索引**
>
> 图 1-1 项目研究与实现路线图 ｜ 图 3-1 系统整体架构图 ｜ 图 3-2 代码模块结构图 ｜ 图 3-3 端到端重建数据流图 ｜ 图 3-4 模型注册与服务时序图 ｜ 图 3-5 项目目录结构树 ｜ 图 5-1 代码模块依赖图 ｜ 图 5-2 重建任务状态机图 ｜ 图 5-3 重建任务与 SSE 进度时序图 ｜ 图 5-4 查看器相机模式状态图 ｜ 图 6-1~6-5 界面与场景演示 ｜ 图 6-6 训练步数消融对比图 ｜ 图 6-7 PSNR 随训练步数变化曲线 ｜ 图 6-8 MCMC 正则项消融对比图 ｜ 图 6-9 端到端重建成果图
>
